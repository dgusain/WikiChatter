...
response_template = """
You are an expert researcher with knowledge based on the provided document summaries. Using the summaries below, generate an accurate and concise answer to the user's question. After the answer, list the references used in the format 'References: Doc1, Doc2, Doc3'.
...
def get_response():
    try:
        ...
        if 'query' in intent:
            ...
            if summaries:
                # Aggregate summaries and collect reference IDs
                summaries_text = "\n".join([
                    f"Doc {i+1} (Doc ID: {doc['doc_id']}): {doc['summary']}" 
                    for i, doc in enumerate(summaries)
                ])
                reference_ids = ", ".join([doc['doc_id'] for doc in summaries])
